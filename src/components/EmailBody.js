...
class EmailBody extends React.Component {
    render(){
        let slideBox = this.props.slide?
            'border-1px width-50pct bg-white position-absolute top-5 left-45 transition box-sizing shadow' :
            'border-1px width-50pct bg-white position-absolute top-5 left-150 transition box-sizing shadow'
        
        return(
            <div className={slideBox}>
                {this.props.emailChecked.map((email, index) => {
                    let receiver = email.to ? email.to.join(', ') : ''
                    let emailBodyClass = email? 
                        'padding-top-5px padding-bottom-10px padding-x-10px border-b word-wrap' : 'display-none'
